...
<nav class="navbar navbar-expand-lg main-navbar">
    ...
    @auth
        <ul class="navbar-nav navbar-right">
            @if ($attributes->get('messages',true))
            ...
            @if ($attributes->get('notifications',true))
                <x-layout-navbar-dropdown dropdown-list="dropdown-list" :toggle="true" :beep="true" :right="true" link-style="notification-toggle" class="dropdown dropdown-list-toggle">
                        <x-slot name="header">
                            @lang('layout.notifications')
                            <div class="float-right"> <a href="#">@lang('layout.mark_all_as_read')</a> </div>
                        </x-slot>
                        ...
                        <div class="dropdown-list-content dropdown-list-icons">
                            @forelse (auth()->user()->notifications()->latest()->limit(4)->get() as $notification)
                                <x-layout-notification-item-dropdown href="#" :read="true" >
                                    ...
                                    <x-slot name="message"> {{ $notification->data['title'] ?? __('TITLE') }} </x-slot>
                                    <x-slot name="time"> {{ $notification->created_at->diffForHumans() }} </x-slot>
                                </x-layout-notification-item-dropdown>
                            @empty
                                <a class="dropdown-item text-center"> <h3> @lang('layout.empty') </h3> </a>
                            @endforelse
                        </div>
                        <x-slot name="footer">
                            <a href="#">@lang('layout.view_all') <i class="fas fa-chevron-right"></i></a>
                        </x-slot>
                    </x-layout-navbar-dropdown>
            @endif
